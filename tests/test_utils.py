...
def test_random_generation_with_fixed_seed():
    ...
    assert np.allclose(np_array_generated,
                       np_array_expected), "Generated numpy array is not ok."

    torch_array_generated = torch.rand((2, 2))
    torch_array_expected = torch.tensor([[0.4962566, 0.7682218],
                                         [0.08847743, 0.13203049]])
    assert torch.allclose(
        torch_array_generated,
        torch_array_expected), "Generated torch array is not ok."
